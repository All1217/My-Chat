...
public class OrchestratorWorkflow {
     ...
     * @param instruction 给 Worker 的子任务；finish 时可为最终答复
     */
    ...
    public NextAction decideNext(
        ...
        String prompt = """
                你是任务编排器（Orchestrator）。根据用户目标与已完成步骤，决定下一步动作。
                ...
                - finish：任务已完成，可给出最终答复（写在 instruction）

                规则：
                - 复杂任务拆成多步；每步 instruction 只描述当前 Worker 要做的事。
                - 若历史观察已足够回答用户，选 finish，并在 instruction 写出完整最终答复。
                ...
                按以下 JSON 返回（不要 markdown 代码块）：
                \\{
                  "reasoning": "简要理由",
                  "nextAction": "retrieve_kb|file|search|general|finish",
                  "instruction": "给 Worker 的子任务或最终答复"
                \\}
                """;
